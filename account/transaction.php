...
<head>
    ...
    <title>HOG - Riwayat Transaksi</title>
</head>
...
<body>
    <div class="container-fluid mt-5 pt-5">
        <div class="row">
            <nav id="sidebar"
                class="bg-dark col-md-3 col-lg-2 d-md-block border border-start-0 border-2 border-secondary shadow-lg sidebar">
            ...
            if ($result->num_rows > 0) {
              while ($row = $result->fetch_assoc()) {
                ...
                ?>
                        <a href="invoice.php?id=<?= $row['id_transaksi'] ?>&&ctg=<?= $row['genre'] ?>"
                            class="list-group-item list-group-item-dark list-group-item-action">
                            <img src="data:../image;base64,<?= base64_encode($row['gambar']) ?>" alt="logo"
                                class="me-3 img-thumbnail float-start rounded-circle-img">
